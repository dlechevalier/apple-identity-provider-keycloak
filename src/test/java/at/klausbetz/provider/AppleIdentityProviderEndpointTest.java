 ...
 *
 * <p>Note: jakarta.ws.rs.core.Response cannot be mocked without a JAX-RS RuntimeDelegate
 * implementation on the classpath. The callback mock methods return null by default, which is
 * sufficient for interaction verification.
 */
...
class AppleIdentityProviderEndpointTest {
  ...
  void setUp() {
    ...
    endpoint =
        new AppleIdentityProviderEndpoint(appleIdentityProvider, realm, callback, event, session);
  }
  ...
  void authResponse_withAuthorizationCode_preparesSecretAndExchangesToken() throws Exception {
    ...
    when(callback.getAndVerifyAuthenticationSession(state)).thenReturn(authSession);
    when(appleIdentityProvider.sendTokenRequest(
    ...
    verify(appleIdentityProvider)
        .sendTokenRequest(
            eq("auth-code-123"), eq("com.example.app"), isNull(), eq(authSession), anyString());
    verify(callback).authenticated(federatedIdentity);
  }

  @Test
  void authResponse_withAuthorizationCodeAndUserData_passesUserToSendTokenRequest()
      throws Exception {
    String state = createValidState();
    BrokeredIdentityContext federatedIdentity = mock(BrokeredIdentityContext.class);
    when(callback.getAndVerifyAuthenticationSession(state)).thenReturn(authSession);
    ...
    verify(appleIdentityProvider)
        .sendTokenRequest(
            eq("auth-code-123"), eq("com.example.app"), eq(userJson), eq(authSession), anyString());
  }
